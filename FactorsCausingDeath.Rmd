...
```{r}
Factors = c('HCT','coma','shock','LPAR_pct','BD','poedema','convulsions','BUN')
```
...
modshock=gam(outcome ~ shock + s(studyID, bs='re') + s(country, bs='re'),
             data = Complete_Leg_data, family='binomial')
modLPAR=gam(outcome ~ s(LPAR_pct) + s(studyID, bs='re') + s(country, bs='re'),
            data = Complete_Leg_data, family='binomial')
...
modBUN=gam(outcome ~ s(BUN) + s(studyID, bs='re') + s(country, bs='re'),
           data = Complete_Leg_data, family='binomial')
models = list(modHCT=modHCT,
              modcoma=modcoma,
              modshock=modshock,
              modLPAR=modLPAR,
              modBD=modBD,
              modpoedema=modpoedema,
              modconv=modconv,
              modBUN=modBUN) 
```

```{r}
par(las=1, bty='n', mfrow=c(3,3))

#************** LPAR
predsLPAR = predict(modLPAR, newdata = data.frame(LPAR_pct= seq(-3,3,by=.1), 
                                                  studyID='AQ', 
                                                  country='Thailand'), 
                    exclude = c("s(country)","s(studyID)"), 
                    type='response', se.fit=T)
plot(seq(-3,3,by=.1), 100*predsLPAR$fit, ylab='%', xlab='LPAR', 
     type='l', lwd=3,ylim = c(0,100))
lines(seq(-3,3,by=.1), 100*predsLPAR$fit + 100*2*predsLPAR$se.fit, lty=2,lwd=2)
lines(seq(-3,3,by=.1), 100*predsLPAR$fit - 100*2*predsLPAR$se.fit,lty=2,lwd=2)
abline(h = min(100*predsLPAR$fit), lwd=2, lty=3)
#************** BD
predsBD = predict(modBD, newdata = data.frame(BD= -22:35, 
                                              studyID='AQ', 
                                              country='Thailand'), 
                  exclude = c("s(country)","s(studyID)"), 
                  type='response', se.fit=T)
plot(-22:35, 100*predsBD$fit, ylab='%', xlab='base deficit', 
     type='l', lwd=3,ylim = c(0,100))
lines(-22:35, 100*predsBD$fit + 100*2*predsBD$se.fit, lty=2,lwd=2)
lines(-22:35, 100*predsBD$fit - 100*2*predsBD$se.fit,lty=2,lwd=2)
abline(h = min(100*predsBD$fit), lwd=2, lty=3)
#************** HCT
predsHCT = predict(modHCT, newdata = data.frame(HCT=5:45, 
                                                studyID='AQ', 
                                                country='Thailand'), 
                   exclude = c("s(country)","s(studyID)"), 
                   type='response', se.fit=T)
plot(5:45, 100*predsHCT$fit, ylab='%', xlab='Haematocrit', 
     type='l', lwd=3,ylim = c(5,25))
lines(5:45, 100*predsHCT$fit + 100*2*predsHCT$se.fit, lty=2,lwd=2)
lines(5:45, 100*predsHCT$fit - 100*2*predsHCT$se.fit,lty=2,lwd=2)
abline(h = min(100*predsHCT$fit), lwd=2, lty=3)
#************** BUN
predsBUN = predict(modBUN, newdata = data.frame(BUN=2:160, 
                                                studyID='AQ', 
                                                country='Thailand'), 
                   exclude = c("s(country)","s(studyID)"), 
                   type='response', se.fit=T)
plot(2:160, 100*predsBUN$fit, ylab='%', xlab='BUN', 
     type='l', lwd=3,ylim = c(0,100))
lines(2:160, 100*predsBUN$fit + 100*2*predsBUN$se.fit, lty=2,lwd=2)
lines(2:160, 100*predsBUN$fit - 100*2*predsBUN$se.fit,lty=2,lwd=2)
abline(h = min(100*predsBUN$fit), lwd=2, lty=3)
#**************  Coma
predscoma = predict(modcoma, newdata = data.frame(coma=0:1, 
                                                  studyID='AQ', 
                                                  country='Thailand'), 
                    exclude = c("s(country)","s(studyID)"), 
                    type='response', se.fit=T)
plot(0:1, 100*predscoma$fit, ylab='%', xlab='', 
     type='l', lwd=3, ylim = c(0,26), xaxt='n')
axis(1, at=0:1, labels = c('No Coma','Coma'))
lines(0:1, 100*predscoma$fit + 100*2*predscoma$se.fit, lty=2,lwd=2)
lines(0:1, 100*predscoma$fit - 100*2*predscoma$se.fit,lty=2,lwd=2)
#************** Convulsions
predsconv = predict(modconv, newdata = data.frame(convulsions=0:1, 
                                                  studyID='AQ', 
                                                  country='Thailand'), 
                    exclude = c("s(country)","s(studyID)"), 
                    type='response', se.fit=T)
plot(0:1, 100*predsconv$fit, ylab='%', xlab='', 
     type='l', lwd=3, ylim = c(5,30), xaxt='n')
axis(1, at=0:1, labels = c('No convulsions','Convulsions'))
lines(0:1, 100*predsconv$fit + 100*2*predsconv$se.fit, lty=2,lwd=2)
lines(0:1, 100*predsconv$fit - 100*2*predsconv$se.fit,lty=2,lwd=2)
#************** Shock
predsshock = predict(modshock, newdata = data.frame(shock=0:1, 
                                                    studyID='AQ', 
                                                    country='Thailand'), 
                     exclude = c("s(country)","s(studyID)"), 
                     type='response', se.fit=T)
plot(0:1, 100*predsshock$fit, ylab='%', xlab='', 
     type='l', lwd=3, ylim = c(5,30), xaxt='n')
axis(1, at=0:1, labels = c('No shock','Shock'))
lines(0:1, 100*predsshock$fit + 100*2*predsshock$se.fit, lty=2,lwd=2)
lines(0:1, 100*predsshock$fit - 100*2*predsshock$se.fit,lty=2,lwd=2)
#************** Pulmonary Oedema
predspoedema = predict(modpoedema, newdata = data.frame(poedema=0:1, 
                                                        studyID='AQ', 
                                                        country='Thailand'), 
                       exclude = c("s(country)","s(studyID)"), 
                       type='response', se.fit=T)
plot(0:1, 100*predspoedema$fit, ylab='%', xlab='', 
     type='l', lwd=3, ylim = c(5,40), xaxt='n')
axis(1, at=0:1, labels = c('No PO','PO'))
lines(0:1, 100*predspoedema$fit + 100*2*predspoedema$se.fit, lty=2,lwd=2)
lines(0:1, 100*predspoedema$fit - 100*2*predspoedema$se.fit,lty=2,lwd=2)
```
...
mod_full_GLM = glmer(outcome ~ HCT + LPAR_pct + AgeInYear + coma + convulsions +
                       poedema + log10(BUN) + BD + drug_AS + shock +
                       (1 | studyID) + (1 | country),
                     data = Complete_Leg_data, family=binomial)
...
```{r}
#*********HCT
HCT_CFs = array(dim = c(3, length(x_hcts)))
for(i in 1:length(x_hcts)){
  mydata = Complete_Leg_data
  mydata$HCT=x_hcts[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  HCT_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********BD
BD_CFs = array(dim = c(3, length(x_bd)))
for(i in 1:length(x_bd)){
  mydata = Complete_Leg_data
  mydata$BD=x_bd[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  BD_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********BUN
BUN_CFs = array(dim = c(3, length(x_BUN)))
for(i in 1:length(x_BUN)){
  mydata = Complete_Leg_data
  mydata$BUN=x_BUN[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  BUN_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********LPAR
LPAR_CFs = array(dim = c(3, length(x_Lpars)))
for(i in 1:length(x_Lpars)){
  mydata = Complete_Leg_data
  mydata$LPAR_pct=x_Lpars[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  LPAR_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********Coma
coma_CFs = array(dim = c(3, length(x_comas)))
for(i in 1:length(x_comas)){
  mydata = Complete_Leg_data
  mydata$coma=x_comas[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  coma_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********Shock
shock_CFs = array(dim = c(3, length(x_shock)))
for(i in 1:length(x_shock)){
  mydata = Complete_Leg_data
  mydata$shock=x_shock[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  shock_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********poedema
PO_CFs = array(dim = c(3, length(x_poedema)))
for(i in 1:length(x_poedema)){
  mydata = Complete_Leg_data
  mydata$poedema=x_poedema[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  PO_CFs[,i] = quantile(ys, probs=myquantiles)
}
#*********Convulsions
Conv_CFs = array(dim = c(3, length(x_conv)))
for(i in 1:length(x_conv)){
  mydata = Complete_Leg_data
  mydata$convulsions=x_conv[i]
  ys = 100*predict(mod_full_GLM, newdata = mydata, re.form=NA, type='response')
  Conv_CFs[,i] = quantile(ys, probs=myquantiles)
}
```
...
plot(x_hcts,HCT_CFs[2,], xlim=c(4,45), ylab='Predicted probability of death', 
     xlab='Haematocrit (%)', ylim=c(0,20), lty=1, lwd=3, type='l')
lines(x_hcts, HCT_CFs[1,], lty=2, lwd=2)
lines(x_hcts, HCT_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_bd,BD_CFs[2,], ylab='', 
     xlab='BD', ylim=c(0,80), lty=1, lwd=3, type='l')
lines(x_bd, BD_CFs[1,], lty=2, lwd=2)
lines(x_bd, BD_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_BUN,BUN_CFs[2,], ylab='', 
     xlab='bun', ylim=c(0,40), lty=1, lwd=3, type='l')
lines(x_BUN, BUN_CFs[1,], lty=2, lwd=2)
lines(x_BUN, BUN_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_Lpars,LPAR_CFs[2,], ylab='', 
     xlab='lpar', ylim=c(0,10), lty=1, lwd=3, type='l')
lines(x_Lpars, LPAR_CFs[1,], lty=2, lwd=2)
lines(x_Lpars, LPAR_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_comas,coma_CFs[2,], ylab='', 
     xlab='coma', ylim=c(0,20), lty=1, lwd=3, type='l')
lines(x_comas,coma_CFs[1,], lty=2, lwd=2)
lines(x_comas,coma_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_conv,Conv_CFs[2,], ylab='', 
     xlab='convulsions', ylim=c(0,20), lty=1, lwd=3, type='l')
lines(x_conv,Conv_CFs[1,], lty=2, lwd=2)
lines(x_conv,Conv_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_poedema,PO_CFs[2,], ylab='', 
     xlab='poedema', ylim=c(0,20), lty=1, lwd=3, type='l')
lines(x_poedema,PO_CFs[1,], lty=2, lwd=2)
lines(x_poedema,PO_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)

plot(x_shock,shock_CFs[2,], ylab='', 
     xlab='shock', ylim=c(0,20), lty=1, lwd=3, type='l')
lines(x_shock,shock_CFs[1,], lty=2, lwd=2)
lines(x_shock,shock_CFs[3,], lty=2, lwd=2)
abline(h=overall_median_mortality, lwd=3, col='blue',lty=2)
```
