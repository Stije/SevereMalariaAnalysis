...
predsBD = predict(modBD, newdata = data.frame(BD= x_bd, 
                  ...
                  exclude = c("s(country)","s(studyID)"), 
                  se.fit=T)
plot(x_bd, 100*inv.logit(predsBD$fit), ylab='mortality (%)', xlab='Base deficit (mEq/L)', 
...
predsHCT = predict(modHCT, 
                   ...
                   exclude = c("s(country)","s(studyID)"), 
                   se.fit=T)
plot(x_hcts, 100*inv.logit(predsHCT$fit), ylab='mortality (%)', xlab='Haematocrit (%)', 
...
predscoma = predict(modcoma, newdata = data.frame(coma=x_comas, 
                    ...
                    exclude = c("s(country)","s(studyID)"), 
                    se.fit=T)
plot(x_comas, 100*inv.logit(predscoma$fit), ylab='mortality (%)', xlab='', 
     type='p', lwd=3, ylim = c(0,28), xaxt='n', pch=19, xlim = c(-.25,1.25))
axis(1, at=x_comas, labels = c('No Coma','Coma'), tick = F)
...
arrows(x0 = x_comas[1],y0 = ys[1], x1 = x_comas[1], y1 = ys[3],
       angle = 90,lwd=6,code = 3,length=.2,col = rgb(0, 0, 1,0.5))
...
predsconv = predict(modconv, newdata = data.frame(convulsions=x_conv, 
                    ...
                    exclude = c("s(country)","s(studyID)"), 
                    se.fit=T)
plot(x_conv, 100*inv.logit(predsconv$fit), ylab='mortality (%)', xlab='', 
     type='p', lwd=3, ylim = c(0,30), xaxt='n',pch=19, xlim = c(-.25,1.25))
axis(1, at=x_conv, labels = c('No seizures','Seizures'),tick = F)
...
arrows(x0 = x_conv[1],y0 = ys[1], x1 = x_conv[1], y1 = ys[3],angle = 90,
       lwd=6,code = 3,length=.2,col = rgb(0, 0, 1,0.5))
...
predspoedema = predict(modpoedema, 
                       ...
                       exclude = c("s(country)","s(studyID)"), 
                       se.fit=T)
plot(x_poedema, 100*inv.logit(predspoedema$fit), ylab='mortality (%)', xlab='', 
     type='p', lwd=3, ylim = c(0,55), xaxt='n',pch=19, xlim = c(-.25,1.25))
axis(1, at=x_poedema, labels = c('No Pulmonary\nOedema','Pulmonary\nOedema'),tick = F)
